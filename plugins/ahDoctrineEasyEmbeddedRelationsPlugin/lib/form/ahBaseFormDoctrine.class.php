...
abstract class ahBaseFormDoctrine extends sfFormDoctrine
{
  ...
  protected function isNewFormEmpty(array $values, array $keys)
  {
    ...
    // Patch for i18n
    $emptyFields = array();
    $this->checkValueArray($values, $emptyFields, $keys['considerNewFormEmptyFields']);

    if (count($emptyFields) === count($keys['considerNewFormEmptyFields']))
    {
      return true;
    }
    return false;
  ...
  // Patch for i18n
  private function checkValueArray($values, &$emptyFields, $keys)
  {
    foreach ($values as $key => $value)
    {
      if (is_array($value) && in_array($key, $keys))
      {
        if (isset($emptyFields[$key]))
          unset($emptyFields[$key]);
        if (count($value) === 0)
          $emptyFields[$key] = 1;
        elseif (array_key_exists('tmp_name', $value) && $value['tmp_name'] === '' && $value['size'] === 0)
          $emptyFields[$key] = 1;
        // Fix for date field
        elseif (count($value) == 3 
                && array_key_exists('month', $value) && trim($value['month']) === ''
                && array_key_exists('day', $value)   && trim($value['day']) === ''
                && array_key_exists('year', $value)  && trim($value['year']) === '') 
          $emptyFields[$key] = 1;
      }
      elseif (is_array($value))
        $this->checkValueArray($value, $emptyFields, $keys);
      elseif (in_array($key, $keys) && '' === trim($value))
        $emptyFields[$key] = 1;
      elseif (in_array($key, $keys) && '' != trim($value))
        if (isset($emptyFields[$key]))
          unset($emptyFields[$key]);
